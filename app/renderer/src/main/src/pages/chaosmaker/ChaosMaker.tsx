...
const groups: ChaosMakerRuleGroup[] = [
    {
        "Title": i18next.t("攻击 Struts2"),
        "Description": i18next.t("针对基于Java Web的Struts框架的应用程序的安全攻击，可能导致信息泄露、权限提升、远程代码执行等问题。"),
        "Keywords": "struts",
        "Protocols": [
            "http",
        ],
        "Solution": i18next.t("为了防止Java Web Struts攻击，请确保及时更新Struts框架至最新版本，遵循安全编码实践，对输入进行有效的验证与过滤，并定期进行安全审计。")
    },
    {
        "Title": i18next.t("攻击 SpringCloud"),
        "Description": i18next.t("针对基于Java Web的SpringMVC框架的应用程序的安全攻击，可能导致信息泄露、权限提升、远程代码执行等问题。"),
        "Keywords": "spring",
        "Protocols": [
            "http",
        ],
        "Solution": i18next.t("为了防止Java Web SpringMVC框架攻击，请确保及时更新SpringMVC框架至最新版本，遵循安全编码实践，对输入进行有效的验证与过滤，并定期进行安全审计。")
    },
    {
        "Title": i18next.t("ICMP 隧道回连"),
        "Description": i18next.t("ICMP隧道回连是一种利用ICMP协议的数据包在网络中建立隐蔽的通信通道的技术。攻击者可能利用这种技术绕过防火墙，实现远程控制、数据泄露等目的。"),
        "Keywords": "icmp",
        "Protocols": [
            "icmp"
        ],
    ...
    },
    {
        ...
        "Solution": i18next.t("为了防止DNS协议下挖矿外连攻击，建议采取以下措施：1. 安装并更新有效的安全防护软件，以便检测和移除恶意挖矿程序；2. 对DNS流量进行监控，分析异常请求和数据传输；3. 限制或禁止未经授权的设备访问内部网络；4. 增强DNS服务器安全，限制未经授权的DNS解析；5. 定期进行安全审计，检查系统漏洞。")
    },
    {
        "Title": i18next.t("攻击协同办公套件"),
        "Description": i18next.t("对常见的OA系统万户Ezoffice、Tongda OA、红帆OA、Weaver e-bridge、FineReport、Weaver e-cology、Weaver e-office、Weaver e-mobile、YonYou NC等产品的历史漏洞进行攻击"),
        "Keywords": "oa",
        "Protocols": [
            "http"
        ],
        "Solution": i18next.t("及时升级软件版本：对于已知的漏洞,协同办公软件厂商通常会发布修复程序,用户应该及时升级软件版本以避免漏洞被攻击者利用。配置安全策略：协同办公软件一般包括许多功能和模块,用户可以根据自身需求和安全要求配置相应的安全策略,例如禁止外网访问、限制文件上传大小、配置防火墙等。")
    },
    {
    ...
    {
        "Title": i18next.t("攻击中间件服务器"),
        "Description": i18next.t("对常见的IIS、apache、Nginx、Weblogic、Tomcat 、jboss等进行历史漏洞进行攻击"),
        "Keywords": "iis,apache,nginx,weblogic,tomcat,jboss",
        "Protocols": [],
        "Solution": i18next.t("及时升级软件版本：对于已知的漏洞,Web服务器的开发团队通常会发布修复程序,用户应该及时升级软件版本以避免漏洞被攻击者利用。启用安全配置：Web服务器包括许多配置选项,用户应该根据自身需求和安全要求配置相应的安全策略,如关闭不必要的服务、启用HTTPS、配置访问控制等。")
    },
    {
        "Title": i18next.t("攻击IOT设备"),
        "Description": i18next.t("对常见的IPCamera、iCatch摄像头、GoAhead摄像头、avtech摄像头、ACTI Camera、5MP Network Camera、海康威视摄像头等进行历史漏洞进行攻击"),
        "Keywords": i18next.t("camara,ipcamera,icatch,goadhead,avtech,acti,5mp,海康威视,hiwatch,hikvision"),
        "Protocols": [],
        "Solution": i18next.t("及时固件版本：对于已知的漏洞,设备厂商通常会发布补丁进行修复。尽量将设备部署在内网环境")
    },
    {
        "Title": i18next.t("攻击邮件服务器"),
        "Description": i18next.t("对常见的exchange、亿邮、Coremail、anymacro、华为 anymail、webmail、zimbra等进行历史漏洞进行攻击"),
...
export const ChaosMakerPage: React.FC<ChaosMakerPageProp> = (props) => {
    ...
    return <div style={{display: "flex", flexDirection: "column", width: "100%", height: "100%"}}>
        <PageHeader style={{width: "100%"}} title={"Breach & Attack Simulator Playbook"} subTitle={i18next.t("入侵与攻击模拟剧本管理")}
                    extra={<div>
                        <YakitButton onClick={() => {
                            const d = showDrawer({
                                title: i18next.t("导入规则"),
                                width: "70%",
                                maskClosable: false,
                                content: (
