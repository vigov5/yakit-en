...
export const YakExecutorParams: React.FC<YakExecutorParamsProp> = (props) => {
    ...
    const deleteButton = (i: YakExecutorParam) => {
        return <Popconfirm
            title={i18next.t("确认删除该参数？")} onConfirm={() => deleteParams(i.Key)}
        >
            <Button
    ...
    return <div style={{width: "100%"}}>
        <Form style={{width: "100%", marginLeft: 8, marginBottom: 14}}>
            <Form.Item
                label={i18next.t("最终执行 Yak 命令行")} style={{width: "100%"}}
                help={<>
...
const Param: React.FC<ParamProp> = (props) => {
    ...
    return <>
        ...
        >
            <SelectOne label={i18next.t("参数类型")} data={[
                {value: true, text: i18next.t("Bool 类型")},
                {value: false, text: i18next.t("普通参数")},
            ]} value={isBool} setValue={setIsBool}/>
            <InputItem label={i18next.t("参数名")} value={key} setValue={setKey} required={true}/>
            {!isBool && <InputItem
                label={i18next.t("参数值")} value={value} setValue={setValue} required={true}
            />}
            <Form.Item label={" "} colon={false}>
                <Button type={"primary"} htmlType={"submit"}>{i18next.t("添加参数")}</Button>
            </Form.Item>
        </Form>
    </>
};