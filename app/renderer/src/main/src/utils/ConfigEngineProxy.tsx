...
export const ConfigEngineProxy: React.FC<ConfigEngineProxyProp> = (props) => {
    ...
    >
            ...
            <Alert
                ...
                message={<>
                    <Space direction={"vertical"}>
                        <Space>
                            <div>{i18next.t("当前引擎代理为：")}
                            </div>
                            <Tag color={"red"}>{proxy}</Tag>
                            <Button type={"link"} icon={<ReloadOutlined/>} onClick={update}/>
                        </Space>
                        <div>{i18next.t("本配置将会对绝大部分 Yak 插件自动生效，如果在扫描模块中配置代理，一般来说，配置的代理将会自动覆盖这个配置。")}
                        </div>
                    </Space>
                </>}
            />
        ...
        <InputItem
            label={i18next.t("代理")} value={proxy} setValue={setProxy} autoComplete={historyProxy}
            help={i18next.t("例如 http://127.0.0.1:7890 或 socks://127.0.0.1:7890 等配置均可")}
        />
        <Form.Item colon={false} label={" "}>
            <Button loading={loading} type="primary" htmlType="submit">{i18next.t("更新引擎代理")} </Button>
        </Form.Item>
    </Form>
};
